...
describe('@vx-foundation/benchmark', () => {
  ...
  it('compares results to VX', () => expect(compareFrameworkResults([{ schema: 'https://vx.veelv.site/schemas/benchmark-result/v1', suiteVersion: 1, scenario: 'lists', identity: { framework: 'vx', version: '1', adapterVersion: '1', lockfileIntegrity: 'x' }, environment, warmupIterations: 0, measuredIterations: 1, samples: [{ value: 1, metric: 'duration-ms' }], metadata: {} }])[0]?.winner).toBe('vx'));
});
